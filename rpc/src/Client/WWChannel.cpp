...
void WWChannel::CallMethod(const google::protobuf::MethodDescriptor * method,
                          ...
                          google::protobuf::Closure * done)
{
    ...
    // 向 Zookeeper 查询节点
    ServiceDiscovery & discovery = ServiceDiscovery::getServiceDiscovery("127.0.0.1", "2181");
    ...
    // 创建一个 TCP 连接
    muduo::net::TcpClient client(&_Event_loop, serverAddr, "Channel");

    // 设置回调函数
    client.setConnectionCallback(std::bind(&WWChannel::onConnection, this, std::placeholders::_1));
    client.setMessageCallback(std::bind(&WWChannel::onMessage, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3));
...
void WWChannel::onConnection(const muduo::net::TcpConnectionPtr & conn)
{
    if (conn->connected()) {
        // 连接上服务端，准备发送序列化请求
...
void WWChannel::onMessage(const muduo::net::TcpConnectionPtr & conn, muduo::net::Buffer * buffer, muduo::Timestamp receive_time)
{
