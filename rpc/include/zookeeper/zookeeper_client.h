...
public:
     ...
     * @return 是否创建成功
     */
    bool create_recursive(const std::string & path, const std::string & data, bool ephemeral = false);
     ...
     * @return 是否获取成功
     */
    bool get_children(const std::string & path, std::vector<std::string> & childs);
...
private:
     ...
     */
    [[noreturn]] void _Throw_runtime_error(const std::string & message) const;
};
