...
{
private:
    std::unordered_map<std::string, service_info> _Service_map;     // 服务名-服务信息
...
private:
     ...
     * @brief 消息事件回调函数
     */
    void on_message(const muduo::net::TcpConnectionPtr & conn, muduo::net::Buffer * buffer, muduo::Timestamp receive_time);

    /**
     * @brief 发送响应
     */
    void send_response(const muduo::net::TcpConnectionPtr & conn, google::protobuf::Message * response);
};
